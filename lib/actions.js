...
export function getWikiContent(country) {
    return(dispatch)=>{
        return axios.post(process.env.NEXT_PUBLIC_API_URL + '/api/getWiki', {
            countryName: country
        })
        .then(data=>{dispatch({type:"ADD_WIKI", payload:data})})
    }
}

export function getWeather(coords) {
    return(dispatch)=>{
        return axios.post(process.env.NEXT_PUBLIC_API_URL + '/api/getWeather',{
            latLng: coords
        })
        .then(data=>{dispatch({type:"ADD_WEATHER", payload:data})})
    }
}